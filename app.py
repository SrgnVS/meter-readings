...
@app.route('/')
def index():
    return 'Сервер для передачи показаний работает'
...
def upload():
    try:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', (qr_data, meter_value, photo_url or '', timestamp, created_at_str))
        conn.commit()
        conn.close()
...
def readings_view():
    try:
        ...
        if response.status_code != 200:
            return f"Ошибка получения списка бэкапов: {response.status_code}", 500
        ...
        for file_info in files_sorted:
            ...
            file_url = f"https://cdn.relaxdev.ru{file_path}" if file_path.startswith('/') else f"https://cdn.relaxdev.ru/{file_path}"
